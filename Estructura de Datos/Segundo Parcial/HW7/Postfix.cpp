...
{
    Stack<float> op;
    std::string post  = getPostfix();
    int l = post.size();
    float firstOperand, secondOperand, finalResult;
    float num;
    for(int i = 0; i<l; i++)
    {
        ...
        {
            secondOperand = op.top()->getData();
            op.pop();
            firstOperand = op.top()->getData();
            op.pop();
            op.push(compute(firstOperand, secondOperand, post[i]));
        }
    }
    
    finalResult = op.pop()->getData();
    return finalResult;
}

int Postfix::compute(float num1, float num2, char sign)
{
    float result;
    ...
    switch (sign) {
        ...
        case '-':
            ...
            return result;
        default:
            std::cout<<"Operation couldn't be made"<<std::endl;
            return 0;
    }
    return 0;
}
